...
// ============================================================================
// Parte CLIENT della home (interattivita': sessione, countdown, azioni).
// ============================================================================
// PERCHE' QUESTO FILE ESISTE (DIFETTO 1, verificato in produzione):
// il riquadro "Stato Biblioteca in Tempo Reale" mostrava SEMPRE zero, perche'
// i numeri arrivavano da uno stato client inizializzato a 0 e mai aggiornato
// (vedi la spiegazione estesa in `src/app/page.tsx`, che ora calcola questi
// numeri lato server e li passa qui come prop iniziali `statistiche` /
// `statisticheDisponibili`). Questo file resta un componente client perche'
// il resto della pagina (sessione utente, prenotazione attiva, countdown,
// dialog di check-in) dipende da hook di React che richiedono il browser.
...
import type { StatisticheBiblioteca } from "./page";

interface StatsBiblioteca extends StatisticheBiblioteca {
...
interface HomeClientProps {
  ...
  // Se il calcolo lato server (vedi page.tsx) e' fallito (es. DB
  // irraggiungibile), qui arriva `false`: la UI deve dirlo esplicitamente
  // invece di mostrare zeri che sembrerebbero dati veri.
  statisticheDisponibili: boolean;
}

export function HomeClient({ statistiche, statisticheDisponibili }: HomeClientProps) {
  ...
  // I numeri "di biblioteca" (posti/utenti/occupazione) arrivano gia' pronti
  // dal server (calcolati ad ogni richiesta, vedi `page.tsx`): qui restano
  // fissi per tutta la sessione della pagina. Solo `prenotazioniAttive` resta
  // uno stato client, perche' dipende dall'utente loggato e viene aggiornato
  // dopo il login o dopo una cancellazione (vedi sotto).
  ...
  const quickActions = [
    {
      id: 'prenota',
      title: 'Prenota Posto',
      // Se il server non e' riuscito a calcolare i posti disponibili, non si
      // mostra "0 disponibili": sarebbe di nuovo un numero che sembra vero
      // senza esserlo (il difetto che questa pagina deve correggere).
      description: statisticheDisponibili
        ? `${stats.postiDisponibili} disponibili`
        : 'Posti: dato non disponibile',
      icon: Calendar,
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    {/* Il totale arriva dal DB (calcolato server-side in page.tsx),
                        non e' piu' una costante scritta a mano: prima qui c'era
                        "su 150 totali" fisso, indipendente dal numero vero di
                        posti in biblioteca. */}
                    <p className="text-xs text-muted-foreground">
                      {statisticheDisponibili ? `su ${stats.postiTotali} totali` : "Dato non disponibile"}
                    </p>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    {statisticheDisponibili ? (
                      <p className="text-4xl font-bold text-primary">{stats.utentiAttivi}</p>
                    ) : (
                      <p className="text-4xl font-bold text-muted-foreground">—</p>
                    )}
                    <p className="text-xs text-muted-foreground">
                      {statisticheDisponibili ? "in questo momento" : "Dato non disponibile"}
                    </p>
                  </div>
                  ...
                  <div className="w-full max-w-md mx-auto lg:mx-0">
                    <div className="h-3 bg-secondary rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${statisticheDisponibili ? stats.percentualeOccupazione : 0}%` }}
                      />
                    </div>
                ...
                <div className="relative">
                  <svg className="w-40 h-40 lg:w-48 lg:h-48 transform -rotate-90">
                    ...
                    <circle
                      ...
                      stroke="url(#gradient)"
                      strokeWidth="12"
                      strokeDasharray={`${(statisticheDisponibili ? stats.percentualeOccupazione : 0) * 5.03} 503`}
                      strokeLinecap="round"
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <p className="text-3xl font-bold text-primary">
                        {statisticheDisponibili ? `${stats.percentualeOccupazione}%` : "—"}
                      </p>
